...
export const MAPS = {
  ...
  dust2: {
    id: 'dust2',
    name: 'Пыль 2',
    desc: 'Классика тактических шутеров: лонг, мид, туннели, два сайта (A/B)',
    SIZE: { w: 90, d: 64 },
    COLORS: ['#cdbb92', '#bfae83', '#c96f4a', '#ff4655', '#8a6f4e', '#5a5348'],
    walls: [
      // периметр
      [0, -32, 92, 1.5, 6.5, 1], [0, 32, 92, 1.5, 6.5, 1],
      [-45, 0, 1.5, 66, 6.5, 1], [45, 0, 1.5, 66, 6.5, 1],
      // разделители трёх маршрутов от спавна T (лонг | мид | туннели)
      [14, 14, 1.2, 20, 4.5, 0], [-14, 14, 1.2, 20, 4.5, 0],
      // «длинные двери» (Long doors) — дверной проём в лонге у восточного края
      [22, 6, 16, 1.2, 4, 4],
      // стены сайтов (A справа, B слева), с проходами: кэт от мида и выход из CT
      [13, -7, 1.2, 6, 4.5, 3], [13, -19, 1.2, 6, 4.5, 3],
      [-13, -7, 1.2, 6, 4.5, 3], [-13, -19, 1.2, 6, 4.5, 3],
      // экран в мид-north, прикрывает спавн CT от прямого прострела
      [0, -14, 8, 1.2, 4, 2],
    ],
    crates: [
      // сайт A — дефолт-плент + укрытия (машина/ящики/рамп)
      [26, -14, 2.4, 2.4, 1.0, 4], [22, -11, 2.0, 2.0, 1.9, 5],
      [31, -19, 2.2, 2.2, 1.9, 5], [30, -9, 2.0, 2.0, 0.9, 4],
      // сайт B — плент + бэк-плат
      [-26, -13, 2.4, 2.4, 1.0, 4], [-22, -10, 2.0, 2.0, 1.9, 5],
      [-31, -18, 2.2, 2.2, 1.9, 5], [-30, -9, 2.0, 2.0, 0.9, 4],
      // мид (xbox)
      [0, 4, 2.2, 2.2, 1.4, 4], [0, 4, 1.4, 1.4, 2.6, 5], [0, -6, 2.0, 2.0, 0.9, 4],
      // лонг
      [34, 2, 2.4, 2.4, 1.9, 5], [30, -3, 2.0, 2.0, 0.9, 4],
      // туннели
      [-34, 4, 2.2, 2.2, 1.9, 5], [-22, -2, 2.0, 2.0, 0.9, 4],
      // укрытия у спавна T
      [-9, 20, 2.2, 2.2, 1.4, 5], [9, 20, 2.2, 2.2, 1.4, 5],
      // укрытия у спавна CT
      [-7, -25, 2.0, 2.0, 0.9, 4], [7, -25, 2.0, 2.0, 0.9, 4],
    ],
    stairs: [],
    sites: {
      A: { x: 27, z: -15, w: 16, d: 14 },
      B: { x: -27, z: -14, w: 16, d: 14 },
    },
    spawns: {
      attack: { pts: [[0, 0, 28], [-5, 0, 28], [5, 0, 28], [-10, 0, 27.5], [10, 0, 27.5]], yaw: 0 },
      defend: { pts: [[0, 0, -27], [-5, 0, -27], [5, 0, -27], [-10, 0, -26.5], [10, 0, -26.5]], yaw: Math.PI },
    },
    nav: {
      nodes: [
        [0, 28, 0],     // 0 спавн T
        ...
        [30, 22, 0],    // 2 в лонг
        [-30, 22, 0],   // 3 в туннели
        [35, 12, 0],    // 4 лонг середина
        [36, 0, 0],     // 5 длинные двери
        [30, -9, 0],    // 6 лонг → A
        [27, -15, 0],   // 7 плент A
        [22, -11, 0],   // 8 укрытие A
        [0, 12, 0],     // 9 мид юг
        [0, 2, 0],      // 10 мид (xbox)
        [0, -8, 0],     // 11 мид север
        [9, -12, 0],    // 12 кэт / short A
        [-9, -12, 0],   // 13 дверь мид → B
        [-35, 12, 0],   // 14 туннель середина
        [-30, -2, 0],   // 15 туннель север
        [-26, -10, 0],  // 16 туннель → B
        [-27, -15, 0],  // 17 плент B
        [-22, -11, 0],  // 18 укрытие B
        [0, -27, 0],    // 19 спавн CT
        [9, -24, 0],    // 20 CT → A
        [-9, -24, 0],   // 21 CT → B
        [17, -21, 0],   // 22 A тыл (из CT)
        [-17, -21, 0],  // 23 B тыл (из CT)
      ],
      edges: [
        [0, 1], [0, 2], [0, 3],
        [2, 4], [4, 5], [5, 6], [6, 7], [7, 8],
